...
### Let's make the points maroon, by changing the color parameter
ggplot(mpg, aes(x = displ, y = hwy)) + 
...
## Not always the default bin size will give you the level of detail you want, or it will over split your data. To change this, we use the "bins" parameter with the geom_ function. 
...
## ggplot has a series of pre-programmed themes that help with that. To use them, we need to add another layer to the plot.

ggplot(mpg, aes(x = hwy)) + 
  geom_histogram(bins = 15, color = "turquoise", fill = "paleturquoise") +
  theme_bw()

## If we wanted to plot a categorical variable, we could apply a bar plot instead of a histogram, which will show the count of each category in the variable.
...
## Pretty neat, no?
## Let's look back at our scatter plot now. 
...
## We can also select pre-made palettes from other packages. One that works very well with ggplot is ColorBrewer. Let's get it going.
## We can see this palettes and their properties here: https://colorbrewer2.org/#type=sequential&scheme=BuGn&n=3
...
## Sometimes when we have too many data points, it might be easier to break down the plots by category instead of plotting them all together. This is usually called faceting. 
...
## When we have multiple categorical variables, it might be interest to see how they are associated. 
...
## the labels end up being automatically put in scientific notation. Let's divide the income by a thousand to adjust it. 
...
## Here, width controls the width of the boxplot. When overlaying, 0.1 usually gives the right proportions. 
...
pdf(file = "scatter_ageVincome_time.pdf", height = 4, width = 6)
ggplot(data, aes(x=age, y=income/1000, size = journey_time/60, color = gender)) +
  geom_point(alpha=0.5) +
  scale_size(range = c(1, 20), name="Journey time (hours)") +
  ylim(20,200)
...
scatter_bubble <- ggplot(data, aes(x=age, y=income/1000, size = journey_time/60, color = gender)) +
                    geom_point(alpha=0.5) +
                    scale_size(range = c(1, 20), name="Journey time (hours)") +
                    ylim(20,200)

pdf(file = "scatter_ageVincome_time_2.pdf", height = 4, width = 6)
scatter_bubble
dev.off()
